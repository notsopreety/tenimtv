...
import { CricketMatch } from "@/app/utils/fetchData";
...
export default function CricketMatchCard({ match }: CricketMatchCardProps) {
  ...
  const getStartsIn = (startStr: string, durationHours: number = 8) => {
    try {
      const now = new Date();
      const start = new Date(startStr);
      const end = new Date(start.getTime() + durationHours * 60 * 60 * 1000);
  ...
  const status = getStatus(match.start, match.duration || 8);
        ...
        <div className="flex flex-col items-center justify-center gap-1.5 w-[16%] text-center shrink-0">
          ...
          {status === "UPCOMING" && (
            <span className="text-[10px] font-semibold text-emerald-655 dark:text-emerald-400 mt-1 whitespace-nowrap">
              Starts in: {getStartsIn(match.start, match.duration || 8)}
            </span>
          )}
        </div>
